...
def memoize_to_disk(func, *args):
    ...
    memo_file = os.path.join(memo_dir, f"{hash_key}.json")

    # Check if the result is already memoized
    if os.path.exists(memo_file):
        print(f"Reading from: {os.path.abspath(memo_file)}")
        with open(memo_file, 'r') as f:
            try:
                return json.load(f)
            except json.JSONDecodeError:
                pass  # If the file is corrupted, recompute the result

    # Compute the result and save it to a new file
    result = func(*args)
    with open(memo_file, 'w') as f:
        json.dump(result, f)
    print(f"Saved to: {os.path.abspath(memo_file)}")
